...
export const diagnosisSchema = z.object({
  ...
  observed_problem: z.string().trim().min(1).max(2000),
  concerning_behaviour: z.string().trim().min(2).max(120),
  inspection_surfaces: z.array(z.string().trim().min(1).max(40)).min(1).max(6),
  inspection_details: z.string().trim().min(1).max(2000),
  ninety_day_consequence: z.string().trim().min(1).max(2000),
  current_feeling: z.string().trim().max(2000).default(""),
